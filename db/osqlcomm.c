                        ...
                        struct block_err *err, int *receivedrows)
{
    ...
    case OSQL_RECGENID: {
        ...
        if (gbl_enable_osql_logging) {
            uuidstr_t us;
            logmsg(LOGMSG_DEBUG, "[%llu %s] OSQL_RECGENID %llx (%llu) -> rc = %d\n", rqid, comdb2uuidstr(uuid, us),
                   lclgenid, lclgenid, rc);
        }
        ...
        if (rc != 1) {
            ...
            if (bdberr != RC_INTERNAL_RETRY) {
                reqerrstr(iq, COMDB2_DEL_RC_INVL_KEY, "unable to find genid =%llx rc=%d", lclgenid, bdberr);
            }
