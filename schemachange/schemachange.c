...
static int do_async_schema_change(struct ireq * const iq, sc_arg_t * arg)
{
    int rc = 0;
    struct schema_change_type * const s = iq->sc;
...
int launch_schema_change(struct ireq *iq, tran_type *trans)
{
    ...
    *  forward in order to produce minimal spew
    */
    if (s->kind != SC_PARTIALUPRECS) {
        logmsg(LOGMSG_INFO, "Executing %s\n", s->nothrevent ? "ASYNCHRONOUSLY" : "SYNCHRONOUSLY");
    }

    int rc = s->nothrevent ? do_schema_change_tran(arg) : do_async_schema_change(iq, arg);
...
int sc_list_create(sc_list_t *scl, void *vscs, uuid_t uuid)
{
    ...
    size_t sizes[scl->count];
    void *packed_scs[scl->count];
    struct schema_change_type *sc;
    ...
    LISTC_FOR_EACH(scs, sc, scs_lnk) {
        ...
        /* save schema change */
        if (sc_protobuf) {
            p_buf = buf_no_net_put(packed_scs[i], sizes[i], p_buf, p_buf_end);
            free(packed_scs[i]);
        } else {
