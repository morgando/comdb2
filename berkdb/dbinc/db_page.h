...
#define	TYPE(p)		(((PAGE *)p)->type & TYPE_MASK) // For accessing
...
#define	P_ENTRY(dbp, pg, indx)	((u_int8_t *)pg + P_INP(dbp, pg)[indx * ((dbp->offset_bias == 0) ?  (abort(), 0) : 1) ])

/* Get a pointer to the prefix entry */
#define	P_PFXENTRY(dbp, pg)	(BKEYDATA *)((u_int8_t *)pg + *(db_indx_t *)P_PREFIX(dbp, pg))
...
typedef struct _bkeydata {
	...
	u_int8_t  type;		/*    02: Page type AND DELETE/PFX FLAG. */
	u_int8_t  data[1];	/* Variable length key/data item. */
} BKEYDATA;
...
#define	BKEYDATA_SIZE(len) ALIGN(BKEYDATA_SIZE_FLUFFLESS(len), sizeof(u_int32_t))
#define	BKEYDATA_PSIZE(len) (BKEYDATA_SIZE(len) + sizeof(db_indx_t))
#define	BKEYDATA_PSIZE_PG(pg, len) ((IS_PREFIX(pg) ? BKEYDATA_SIZE_FLUFFLESS(len) : BKEYDATA_SIZE(len)) + sizeof(db_indx_t))
