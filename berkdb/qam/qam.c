	...
	db_pgno_t *pgnop;
{
	...
	PAGEPUT(dbc, mpf, meta, opcode != 0 ? DB_MPOOL_DIRTY : 0, t_ret);
	if (t_ret != 0 && ret == 0)
		ret = t_ret;
	...
	DBC *dbc;
{
	...
	if (cp->recno == first) {
		pg = ((QUEUE *)dbp->q_internal)->q_meta;
		if ((ret =
			__db_lget(dbc, 0, pg, DB_LOCK_WRITE, 0,
			    &metalock)) != 0)
			goto err1;
	...
	db_pgno_t *pgnop;
{
	...
	*pgnop = 0;
	 ...
	 * but not locked.
	 */
	PAGEGET(dbc, mpf, &metapno, 0, &meta, ret);
	if (ret != 0)
		return (ret);
...
retry:	/* Update the record number. */
	...
	if ((!with_delete && is_first) || flags == DB_LAST) {
		if ((ret =
			__db_lget(dbc, 0, metapno, lock_mode, 0,
			    &metalock)) != 0) {
			__LPUT(dbc, lock);
			goto err;
		}
	...
	u_int32_t *countp;
{
	...
	PAGEPUT(dbc, mpf, meta, ret == 0 ? DB_MPOOL_DIRTY: 0, t_ret);
	if (t_ret != 0 && ret == 0)
		ret = t_ret;
	if ((t_ret = __LPUT(dbc, metalock)) != 0 && ret == 0)
		ret = t_ret;
