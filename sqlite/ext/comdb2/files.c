    ...
    for (i = 0; i < pIdxInfo->nConstraint; i++, pConstraint++) {
        ...
        case FILES_COLUMN_FILENAME:
            if (pConstraint->op != SQLITE_INDEX_CONSTRAINT_LIKE) {
                logmsg(LOGMSG_ERROR, "%s:%d: Column %d can only be constrained with 'like'\n",
                       __FILE__, __LINE__, FILES_COLUMN_FILENAME);
                return SQLITE_ERROR;
            }
        ...
        case FILES_COLUMN_CHUNK_SIZE:
            if (pConstraint->op != SQLITE_INDEX_CONSTRAINT_EQ) {
                logmsg(LOGMSG_ERROR, "%s:%d: Column %d can only be constrained with '='\n",
                       __FILE__, __LINE__, FILES_COLUMN_CHUNK_SIZE);
                return SQLITE_ERROR;
            }
