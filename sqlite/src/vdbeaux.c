...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
    case P4_OPFUNC: {
      OpFunc *pOpFunc = pOp->p4.comdb2func;
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
  logmsgf(LOGMSG_USER, pOut, zFormat1, pc, 
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
/*  10 */   SIZE_OF_INT_DSMS,  sizeof(dttz_t),  0,  0,  1,  1,  2,  2,  3,  3,
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
...
#ifdef _SUN_SOURCE
      intv_t scratch;
      memcpy(&scratch, buf, SIZE_OF_INT_DSMS);
      p = &scratch;
#endif
...
static int compareDateTimeInterval(
  int f1,
  int f2,
  int combined_flags,
  const Mem *pMem1,
   ...
   * only a afterwards check for numerics */
  ...
  if( combined_flags&MEM_Interval ){
    if( pMem1->du.tv.type==INTV_DECIMAL_TYPE ){
      ...
      decQuadCompare(&result, (decQuad*)&pMem1->du.tv.u.dec,
                     (decQuad*)&pMem2->du.tv.u.dec, &ctx);

      if( decQuadIsZero(&result) ) return 0;
      if( decQuadIsSigned(&result) ) return -1;
      return 1;
    }else{
      /* best effort */
      if( !(f1&MEM_Interval) ){
  ...
  logmsg(LOGMSG_ERROR, "ERROR: called with bad flags\n");
  return 0;
}
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
#if defined(SQLITE_BUILDING_FOR_COMDB2)
  ...
  */
  ...
  if(combined_flags&MEM_Datetime || combined_flags&MEM_Interval) {
    if( f1&(MEM_Int|MEM_Real) ){
      /* If lhs is a number, convert rhs to a real and compare these 2 numbers.
         This takes care of out-of-range values. */
      sqlite3VdbeMemInit(&copy, pMem1->db, MEM_Null);
      sqlite3VdbeMemCopy(&copy, pMem2);
      sqlite3VdbeMemRealify(&copy);

      f2 = copy.flags;
      pMem2 = &copy;
      combined_flags = f1|f2;
    }else if( f2&(MEM_Int|MEM_Real) ){
      /* If rhs is a number, convert lhs to a real and compare these 2 numbers.
         This takes care of out-of-range values. */
      sqlite3VdbeMemInit(&copy, pMem2->db, MEM_Null);
      sqlite3VdbeMemCopy(&copy, pMem1);
      sqlite3VdbeMemRealify(&copy);
      ...
      if( err==0 )
        return rc;
      /* Type error. If one value is a string, convert the other one to
         a string and then compare. */
      if( f1&MEM_Str ){
        sqlite3VdbeMemInit(&copy, pMem1->db, MEM_Null);
        sqlite3VdbeMemCopy(&copy, pMem2);

        rc = sqlite3VdbeMemStringify(&copy, pColl ? pColl->enc : pMem1->enc, 0);
        if( rc!=SQLITE_OK )
          return rc;
        ...
        rc = sqlite3VdbeMemStringify(&copy, pColl ? pColl->enc : pMem2->enc, 0);
        if( rc!=SQLITE_OK )
          return rc;
  ...
  */
  if( combined_flags&MEM_Str ){
    ...
    */
    ...
    if( pColl ){
#if defined(SQLITE_BUILDING_FOR_COMDB2)
      int rc = vdbeCompareMemString(pMem1, pMem2, pColl, 0);
      if( pMem1==&copy || pMem2==&copy ){
        sqlite3VdbeMemRelease(&copy);
      }
      return rc;
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
  int rc = sqlite3BlobCompare(pMem1, pMem2);
  if( pMem1==&copy || pMem2==&copy ){
    sqlite3VdbeMemRelease(&copy);
  }
  return rc;
  ...
  ** that this routine begins comparing at the second field. */
  ...
  do{
    ...
    if( combined_flags&MEM_Null ){
      ...
      rc = sqlite3MemCompare(&mem1, pRhs,  pPKey2->pKeyInfo->aColl[i]);
    }
    else
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
    /* RHS is an integer */
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
      if( !sqlite3IsFixedLengthSerialType(serial_type) ){
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
      if( serial_type>=10 ){
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
        rc = +1;
    ...
    /* RHS is a blob */
      ...
      testcase( serial_type==12 );
#if defined(SQLITE_BUILDING_FOR_COMDB2)
      if( sqlite3IsFixedLengthSerialType(serial_type) || (serial_type & 0x01) ){
#else /* defined(SQLITE_BUILDING_FOR_COMDB2) */
...
#if defined(SQLITE_BUILDING_FOR_COMDB2)
Mem *sqlite3GetCachedResultRow(
  sqlite3_stmt *pStmt,
  int *nColumns
){
  Vdbe *p = (Vdbe *)pStmt;
  if( p ){
    if (nColumns) *nColumns = p->nResColumn;
    return p->pResultSet;
  }
  return NULL;
}
...
void comdb2SetReplace(Vdbe *v){ v->oeFlag |= Cdb2_OE_Replace; }
void comdb2SetUpdate(Vdbe *v) { v->oeFlag |= Cdb2_OE_Update; }
void comdb2SetIgnore(Vdbe *v) { v->oeFlag |= Cdb2_OE_Ignore; }
void comdb2SetUpsertIdx(Vdbe *v, int idx){ v->upsertIdx = idx; }
int comdb2UpsertIdx(Vdbe *v){ return v->upsertIdx; }
int comdb2ForceVerify(Vdbe *v){ return (v->oeFlag&(Cdb2_OE_Replace|Cdb2_OE_Update)) ? 1 : 0; }
int comdb2IgnoreFailure(Vdbe *v){ return (v->oeFlag&Cdb2_OE_Ignore) ? 1 : 0; }
#endif /* defined(SQLITE_BUILDING_FOR_COMDB2) */
