...
#ifdef LULU2
	fprintf(stdout,
	    "FC: tid %d locker %lu lid %lu addr %lx [pp_allocated %u]\n",
	    dbc->tid, dbc->locker, dbc->lid, dbc, dbc->pp_allocated);
#endif

	if (dbc->pp_allocated) {
		/*fprintf(stderr, "thread %d closed cursor %x !\n", pthread_self(), dbc); */
#ifndef TESTSUITE
		void *p;
...
/*
 * PUBLIC: int __db_c_get_pageindex __P((DBC *, int *, int *));
 */
int
__db_c_get_pageindex(dbc, page, index)
	DBC *dbc;
	int *page;
	int *index;
{
	if (dbc->dbtype != DB_BTREE) {
		logmsg(LOGMSG_ERROR, "__db_c_get_pageindex only for btree\n");
		return -1;
	}

	return __bam_c_get_pageindex(dbc, page, index);
}

/*
 * PUBLIC: int __db_c_get_pagelsn __P((DBC *, DB_LSN *));
 */
int
__db_c_get_pagelsn(dbc, lsn)
	DBC *dbc;
	DB_LSN *lsn;
{
	if (dbc->dbtype != DB_BTREE) {
		logmsg(LOGMSG_ERROR, "__db_c_get_pagelsn only for btree\n");
		return -1;
	}

	return __bam_c_get_pagelsn(dbc, lsn);
}
 ...
 *	comdb2 addition
 *
 * PUBLIC: int __db_c_close_ser __P((DBC *, DBCS *));
 */
int
__db_c_close_ser(dbc, dbcs)
	DBC *dbc;
	DBCS *dbcs;
{
	DBC_INTERNAL *cp;
	...
	/* Serialize the DBC internal structure. */
	switch (dbc->dbtype) {
	case DB_BTREE:
	case DB_RECNO:
		if ((ret = __bam_c_ser(dbc, dbcs)) != 0)
			goto err;
		break;
	case DB_HASH:
	case DB_QUEUE:
	case DB_UNKNOWN:
	default:
		ret = __db_unknown_type(dbc->dbp->dbenv, "__db_c_close_ser",
		    dbc->dbtype);
		goto err;
	}
	...
	/* Serialize common structure */
	dbcsi->pgno = cp->pgno;
	dbcsi->indx = cp->indx;
	dbcsi->lock_mode = cp->lock_mode;
...
/*
 * PUBLIC: int __db_c_getpgno __P((DBC *, db_pgno_t *));
 */
int
__db_c_getpgno(dbc, pgno)
	DBC *dbc;
	db_pgno_t *pgno;
{
	int ret;

	/* Pause the DBC internal structure. */
	switch (dbc->dbtype) {
	case DB_BTREE:
	case DB_RECNO:
		if ((ret = __bam_c_getpgno(dbc, pgno)) != 0)
			goto err;
		break;
	case DB_HASH:
	case DB_QUEUE:
	...
	default:
		ret =
		    __db_unknown_type(dbc->dbp->dbenv, "__db_c_getpgno",
		    dbc->dbtype);
		goto err;
	}
...
/*
 * PUBLIC: int __db_c_getnextpgno __P((DBC *, db_pgno_t *));
 */
int
__db_c_getnextpgno(dbc, pgno)
	DBC *dbc;
	db_pgno_t *pgno;
{
	int ret;
	...
	case DB_BTREE:
	case DB_RECNO:
		if ((ret = __bam_c_getnextpgno(dbc, pgno)) != 0)
			goto err;
		break;
	case DB_HASH:
	case DB_QUEUE:
	case DB_UNKNOWN:
	default:
		ret = __db_unknown_type(dbc->dbp->dbenv, "__db_c_getnextpgno",
		    dbc->dbtype);
		goto err;
	}
...
int
__db_c_getgenids(dbc, genids, num, max)
	DBC *dbc;
	...
	int max;
{
	...
	case DB_BTREE:
	case DB_RECNO:
		if ((ret = __bam_c_getgenids(dbc, genids, num, max)) != 0)
			goto err;
		break;
	case DB_HASH:
	case DB_QUEUE:
	case DB_UNKNOWN:
	default:
		ret = __db_unknown_type(dbc->dbp->dbenv, "__db_c_getgenids",
		    dbc->dbtype);
		goto err;
	}
 ...
 * __db_c_firstleaf --
 *  DBC->c_firstleaf
 *  comdb2 addition: returns the first leaf-page that a SET_RANGE 
 *  lands on before resolving to a valid record.  If the firstleaf 
 *  is the same as the current page, nothing could have sneaked in
 *  as the page hasn't been unlocked.
 *
 * PUBLIC: int __db_c_firstleaf __P((DBC *, db_pgno_t *pgno));
 */
int
__db_c_firstleaf(dbc, pgno)
	DBC *dbc;
	db_pgno_t *pgno;
{
	int ret;
	...
	case DB_BTREE:
	case DB_RECNO:
		if ((ret = __bam_c_firstleaf(dbc, pgno)) != 0)
			goto err;
		break;
	case DB_HASH:
	case DB_QUEUE:
	...
	default:
		ret =
		    __db_unknown_type(dbc->dbp->dbenv, "__db_c_firstleaf",
		    dbc->dbtype);
		goto err;
	}
 ...
 *	comdb2 addition
 *
 * PUBLIC: int __db_c_pause __P((DBC *, DBCPS *));
 */
int
__db_c_pause(dbc, dbcps)
	DBC *dbc;
	DBCPS *dbcps;
{
	int ret;
	...
	if (!F_ISSET(dbc, DBC_PAUSIBLE)) {
		/* Abort so that I can find these */
		abort();
	}
	...
	case DB_BTREE:
	case DB_RECNO:
		if ((ret = __bam_c_pause(dbc, dbcps)) != 0)
			goto err;
		break;
	case DB_HASH:
	case DB_QUEUE:
	...
	default:
		ret =
		    __db_unknown_type(dbc->dbp->dbenv, "__db_c_pause",
		    dbc->dbtype);
		goto err;
	}
...
err:
	/* Close if it's any error other than not-positioned */
 ...
 *	comdb2 addition
 *
 * PUBLIC: int __db_c_unpause __P((DBC *, DBCPS *));
 */
int
__db_c_unpause(dbc, dbcps)
	DBC *dbc;
	DBCPS *dbcps;
{
	int ret;
	...
	/* Unpause the DBC internal structure. */
	switch (dbc->dbtype) {
	case DB_BTREE:
	case DB_RECNO:
		if ((ret = __bam_c_unpause(dbcps, dbc)) != 0)
			goto err;
		break;
	case DB_HASH:
	case DB_QUEUE:
	case DB_UNKNOWN:
	default:
		ret = __db_unknown_type(dbc->dbp->dbenv, "__db_c_unpause",
		    dbc->dbtype);
		goto err;
	}
...
/*
 * PUBLIC: int __db_c_close_nocount __P((DBC *));
 */
int
	...
	u_int32_t flags;
{
	...
	else if ((ret = dbc->c_am_writelock(dbc)) == 0)
	...
	u_int32_t flags;
{
	...
	if (flags == DB_POSITION) {
		...
		dbc_n->flags |=
		    dbc_orig->flags & ~(DBC_OWN_LID | DBC_PAGE_ORDER);
...
#ifdef LULU2
	fprintf(stdout,
	    "CC: tid %d locker %lu lid %lu addr %lx [pp_allocated %u]\n",
	    dbc_n->tid, dbc_n->locker, dbc_n->lid, dbc_n, dbc_n->pp_allocated);
#endif
	...
	DBC **dbcp;
{
	...
	*dbcp = oldopd;
	...
	if ((ret = __db_cursor_int(dbp,
		    dbc_parent->txn, dbtype, root, 1, dbc_parent->locker, &opd,
		    0)) != 0)
		return (ret);
...
/*
 * PUBLIC: int __db_c_get_dup __P((DBC *, DBC **, DBT *, DBT *, u_int32_t));
 */
int
__db_c_get_dup(dbc_arg, dbc_dup, key, data, flags)
	DBC *dbc_arg;
	DBC **dbc_dup;
	DBT *key, *data;
	u_int32_t flags;
...
__db_c_get(dbc_arg, key, data, flags)
	...
	u_int32_t flags;
{
	return __db_c_get_dup(dbc_arg, NULL, key, data, flags);
}
	...
	u_int32_t flags;
{
	...
	for (sdbp = __db_s_first(dbp);
		...
		if ((ret = __db_cursor_int(sdbp, dbc_arg->txn, sdbp->type,
		    PGNO_INVALID, 0, dbc_arg->locker, &sdbc, 0)) != 0)
			goto err;
	...
	int failed;
{
	...
	if (!failed && ret == 0) {
		...
		dbc->nextcount += dbc_n->nextcount;
		dbc->skipcount += dbc_n->skipcount;
#if USE_BTPF
		btpf_copy_dbc(dbc_n, dbc);
#endif
	}
	...
	u_int32_t flags;
{
	...
	if ((ret = __db_cursor_int(pdbp,
	    dbc->txn, pdbp->type, PGNO_INVALID, 0, dbc->locker, &pdbc, 0)) != 0)
		goto err;
	...
	u_int32_t flags;
{
	...
	if (F_ISSET(pdbp, DB_AM_RECNUM)) {
		...
		if ((ret = __db_cursor_int(pdbp, sdbc->txn,
		    pdbp->type, PGNO_INVALID, 0, sdbc->locker, &pdbc, 0)) != 0)
			goto perr;
	...
	DBC *dbc;
{
	...
	if ((ret = __db_cursor_int(pdbp, dbc->txn,
	    pdbp->type, PGNO_INVALID, 0, dbc->locker, &pdbc, 0)) != 0)
		return (ret);
