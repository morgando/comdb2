...
#define PAGEGET(dbc, mpf, pgno, flags, page, ret) \
	(ret = (dbc != NULL && F_ISSET(dbc, DBC_SNAPSHOT)) ? __mempv_fget(mpf, dbc->dbp, *pgno, dbc->snapshot_lsn, dbc->highest_ckpt_commit_lsn, page, flags) : __memp_fget(mpf, pgno, flags, page));

#define PAGEPUT(dbc, mpf, page, flags, ret) \
	(ret = (dbc != NULL && F_ISSET(dbc, DBC_SNAPSHOT)) ? __mempv_fput(mpf, page, flags) : __memp_fput(mpf, page, flags));
