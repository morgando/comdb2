...
extern int __mempv_fget(DB_MPOOLFILE *, DB *, db_pgno_t, DB_LSN, DB_LSN, void *);

#define PAGEGET(dbc, mpf, pgno, flags, page, ret) \
	(ret = (dbc != NULL && F_ISSET(dbc, DBC_SNAPSHOT)) ? __mempv_fget(mpf, dbc->dbp, *pgno, dbc->snapshot_lsn, dbc->highest_ckpt_commit_lsn, page) : __memp_fget(mpf, pgno, flags, page));
