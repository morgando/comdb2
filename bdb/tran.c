...
static int get_modsnap_start_lsn(
    bdb_state_type *bdb_state, bdb_attr_type *bdb_attr,
    int snapshot_epoch, DB_LSN *p_modsnap_start_lsn)
{
    const int is_pit_snapshot = snapshot_epoch;
    const int durable_lsn_is_enabled = bdb_attr_get(bdb_attr, BDB_ATTR_DURABLE_LSNS);

    if (is_pit_snapshot)
    {
        int bdberr = 0;
        const int rc = bdb_get_lsn_context_from_timestamp(bdb_state, snapshot_epoch,
            p_modsnap_start_lsn, 0, &bdberr); 
    ...
    } else if (durable_lsn_is_enabled)
    {
        uint32_t durable_gen;
        const int rc = request_durable_lsn_from_master(
            bdb_state, &(p_modsnap_start_lsn->file), &(p_modsnap_start_lsn->offset), &durable_gen);
    ...
    {
        (void)bdb_get_current_lsn(bdb_state, &(p_modsnap_start_lsn->file), &(p_modsnap_start_lsn->offset));
    }
                        ...
                        unsigned int *last_checkpoint_lsn_offset)
{
    ...
    if (!is_ha_retry) {
        ...
        const int rc = get_modsnap_start_lsn(bdb_state, bdb_attr, snapshot_epoch, &modsnap_start_lsn);
        if (rc) {
