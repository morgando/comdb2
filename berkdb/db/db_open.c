	...
	db_pgno_t meta_pgno;
{
	...
	/* Fill in the type. */
	dbp->type = type;
	dbp->temptable = LF_ISSET(DB_TEMPTABLE) ? 1 : 0;
	if (!dbp->temptable && LF_ISSET(DB_OLCOMPACT))
		dbp->olcompact = 1;
	else
		dbp->olcompact = 0;
	...
	/* temporarily set the hash flag for datafile */
	int clear_hash_flag = 0;

	if (LF_ISSET(DB_DATAFILE) && !F_ISSET(dbp, DB_AM_HASH)) {
	...
	if (dbp->fname == NULL && fname) {
		if ((ret = __os_strdup(dbenv, fname, &dbp->fname)) != 0)
			goto err;
	}
	...
	if (dbp->pgsize > 65536) {
		...
		dbp->offset_bias = dbp->pgsize / 65536;
	} else
		dbp->offset_bias = 1;
	...
	const char *name;
{
	...
	if (dbp->pgsize > 65536) {
		/* If we have large pages, we need a bias factor for
		 * accessing entries. */
		dbp->offset_bias = dbp->pgsize / 65536;
	} else
		dbp->offset_bias = 1;
	...
	DB_TEST_RECOVERY_LABEL return (ret);
}
