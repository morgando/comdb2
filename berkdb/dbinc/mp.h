...
typedef enum {
	...
	DB_SYNC_REMOVABLE_QEXTENT,  /* Remove buffers of removable extent */
	DB_SYNC_LRU		         /* Trickle LRU pages. */
} db_sync_op;
...
struct __mpool {
	...
	/*
	 * We keep a copy of the last lsn that we called memp_trickle on,
	 * thereby preventing useless counting of dirty buffers if we
	 * know that none exist.
	 */
	DB_LSN	  trickle_lsn;		/* Maximum checkpoint LSN. */
};

typedef SH_TAILQ_HEAD(HashTab, __bh) HashTab;
struct __db_mpool_hash {
...
#define BH_PREFAULT	0x080		/* prefault pages */
	...
	u_int16_t	turnstile_empty;
	u_int16_t	empty;
	u_int16_t reader_can_enter;
	u_int16_t ref_reader;
	u_int16_t is_copy;
	u_int32_t lid;
	DBT lkname;
	DB_LOCK lock;
	pthread_rwlock_t rwlock;
	pthread_cond_t turnstile_cond;
